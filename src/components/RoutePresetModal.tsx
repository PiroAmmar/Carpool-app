...
interface RoutePresetModalProps {
  ...
  onConfirm: (data: { name: string; stops: string[] }) => Promise<boolean>;
  onCancel: () => void;
}
...
export function RoutePresetModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const ok = await onConfirm({ name: name.trim(), stops });
  ...
  return (
    <BaseModal
      ...
      badgeText="Route Presets"
      badgeColor="text-warmwhite/40 font-bold"
      title={presetToEdit ? 'Edit Route Preset' : 'New Route Preset'}
      ariaLabel="Route Preset"
    >
