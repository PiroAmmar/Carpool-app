...
function CampusIcon({ className = 'w-3.5 h-3.5' }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true">
      <path d="M3 21h18" />
...
const CATEGORY_ICON: Record<TripCategory, (props: { className?: string }) => React.ReactElement> = {
  home_to_campus: HomeIcon,
  campus_to_home: CampusIcon,
};
...
function TripRow({
  ...
  return (
    <motion.button
      ...
      transition={{ duration: 0.25, ease: [0.23, 1, 0.32, 1], delay: Math.min(index * 0.04, 0.32) }}
      onClick={onClick}
...
export function Sidebar({
  ...
  const tripsByCategory = useMemo(() => {
    ...
    for (const trip of scheduledTrips) {
      groups[categoryOf(trip.direction)].push(trip);
    }
    return groups;
  }, [scheduledTrips]);
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 custom-scrollbar">
        {scheduledTrips.length === 0 ? (
          ...
          <div className="flex flex-col gap-6">
            {(['home_to_campus', 'campus_to_home'] as const).map((category, groupIndex) => {
              const categoryTrips = tripsByCategory[category];
              if (categoryTrips.length === 0) return null;
              const Icon = CATEGORY_ICON[category];
              const isFirstRenderedGroup =
                groupIndex === 0 || tripsByCategory.home_to_campus.length === 0;

              return (
                <div key={category}>
                  {!isFirstRenderedGroup && (
                    <div
                      className="h-px w-full mb-6"
                      style={{
                        backgroundImage:
                          'repeating-linear-gradient(to right, rgba(201,205,211,0.18) 0 8px, transparent 8px 16px)',
                      }}
                      aria-hidden
                    />
                  )}
                  ...
                  <div className="flex items-center gap-2.5 mb-3.5 px-1">
                    <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-chrome/10 border border-chrome/20 text-chrome flex-shrink-0">
                      <Icon className="w-4 h-4" />
                    </span>
                    <p className="text-sm font-bold text-warmwhite tracking-tight leading-none flex-1 min-w-0 truncate">
                      {CATEGORY_LABEL[category]}
                    </p>
                    <span className="font-mono text-[11px] text-warmwhite/35 flex-shrink-0">
                      {categoryTrips.length}
                    </span>
                  </div>

                  <div className="flex flex-col gap-1.5">
                    {categoryTrips.map((trip, i) => (
                      <TripRow
                        key={trip.id}
                        trip={trip}
                        index={i}
                        isActive={trip.id === activeTripId}
                        badge={getTripBadge(trip, bookings, currentUserId, isAdmin)}
                        onClick={() => {
                          if (onSelectTrip) {
